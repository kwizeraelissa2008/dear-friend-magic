...
const AddStudentDialog = ({
  ...
  const handleCsvUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (!cols[idx("gender")]) {
          errors.push(`Row ${i + 1}: Missing gender`);
          continue;
        }
        if (!cols[idx("dateofbirth")]) {
          errors.push(`Row ${i + 1}: Missing DOB`);
          continue;
        }
        if (!cols[idx("studentid")]) {
          errors.push(`Row ${i + 1}: Missing student ID`);
          continue;
        }
        if (seenIds.has(cols[idx("studentid")])) {
          errors.push(
            `Row ${i + 1}: Duplicate student ID ${cols[idx("studentid")]}`,
          );
          continue;
        }
        ...
        students.push({
          ...
          parent_name:
            idx("parentname") >= 0 ? cols[idx("parentname")] : undefined,
          parent_phone:
            idx("parentphone") >= 0 ? cols[idx("parentphone")] : undefined,
  ...
  const handleCsvSubmit = async () => {
    ...
    if (error) {
      if (error.message.includes("duplicate"))
        toast.error("Some students have duplicate admission numbers");
      else toast.error(error.message);
    } else {
