...
const Analytics = () => {
  ...
  const fetchAnalytics = async () => {
    ...
    const { data: incidents } = await supabase
      .from("incidents")
      .select(
        "severity, student_id, created_at, students(class_id, classes(name))",
      );
    ...
    if (incidents) {
      ...
      setSeverityData(
        Object.entries(sevMap).map(([name, count]) => ({ name, count })),
      );
      ...
      incidents.forEach((i) => {
        const month = new Date(i.created_at).toLocaleDateString("en-US", {
          year: "numeric",
          month: "short",
        });
      ...
      <DashboardLayout>
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            Only Principal and Dean of Studies can view analytics.
          </p>
        </div>
      </DashboardLayout>
    );
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Student discipline trends and insights
          </p>
        </div>
