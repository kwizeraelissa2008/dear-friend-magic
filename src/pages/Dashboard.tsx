...
import {
  Users, AlertTriangle, Calendar, FileCheck, Clock,
...
const Dashboard = () => {
  ...
  const statCards = [
    { title: "Total Students", value: stats.totalStudents, description: "Registered in the system", icon: Users, tone: "text-primary", ring: "bg-primary/10" },
    { title: "Pending Incidents", value: stats.pendingIncidents, description: "Awaiting review", icon: AlertTriangle, tone: "text-destructive", ring: "bg-destructive/10" },
    { title: "Active Permissions", value: stats.activePermissions, description: "Currently valid", icon: FileCheck, tone: "text-accent", ring: "bg-accent/10" },
    { title: "Upcoming Events", value: stats.upcomingEvents, description: "Scheduled events", icon: Calendar, tone: "text-primary", ring: "bg-primary/10" },
  ];
  ...
  const quickActions = [
    ...
    { to: "/report", icon: AlertTriangle, label: "Report Incident", show: hasRole("teacher", "discipline_staff") },
    { to: "/reports", icon: FileCheck, label: "Review Reports", show: hasRole("dod") },
    { to: "/analytics", icon: BarChart3, label: "View Analytics", show: hasRole("principal", "dos") },
                ...
                <div className="timeline-track space-y-4">
                  {recentActivity.map(a => {
                    ...
                    return hasRole("principal", "dos", "dod") ? (
                      <Link key={a.id} to="/audit-logs" className="relative block">{Inner}</Link>
                    ) : (
                      <div key={a.id} className="relative">{Inner}</div>
                    );
                  })}
                </div>
              )}
