    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="font-display font-bold text-sm sm:text-base leading-tight truncate">Ecole des Sciences</p>
              <p className="text-[10px] sm:text-xs text-muted-foreground truncate">Byimana — SDMS</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link to="/auth"><Button variant="ghost" size="sm">Sign in</Button></Link>
            <Link to="/auth"><Button size="sm" className="gap-1 rounded-full">Get started <ArrowRight className="w-4 h-4" /></Button></Link>
          </div>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden bg-midnight">
        ...
        <div className="container relative mx-auto px-4 pt-16 pb-20 sm:pt-24 sm:pb-28 text-center">
          ...
          <p className="mt-5 text-base sm:text-lg text-primary-foreground/65 max-w-2xl mx-auto px-2">
            One secure platform for staff and leadership to record incidents, manage permissions,
            and act faster — assisted by a role-aware AI agent.
          </p>
          <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
            <Link to="/auth">
              <Button size="lg" className="w-full sm:w-auto gap-2 rounded-full px-7 shadow-[var(--shadow-glow)]">
          ...
          <p className="mt-5 text-xs text-primary-foreground/45">New accounts require Principal approval before access.</p>
        </div>
      </section>

      {/* Features */}
      <section className="container mx-auto px-4 py-14 sm:py-20">
        <p className="text-center text-xs font-semibold uppercase tracking-[0.18em] text-primary mb-3">Capabilities</p>
        <h2 className="font-display text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">What you can do</h2>
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {features.map(f => {
            const Icon = f.icon;
            return (
              <Card key={f.title} className="group rounded-2xl border-border/70 hover-lift">
      ...
      <section className="border-y border-border/70 bg-muted/40">
        <div className="container mx-auto px-4 py-14 sm:py-20">
          <h2 className="font-display text-2xl sm:text-3xl font-bold text-center mb-2">Roles in the system</h2>
          <p className="text-center text-sm text-muted-foreground mb-10">Every account is approved and assigned a role by the Principal.</p>
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 max-w-3xl mx-auto">
            {roles.map(r => (
              <div key={r.name} className="flex items-start gap-3 p-5 rounded-2xl border border-border/70 bg-card shadow-[var(--shadow-xs)] hover-lift">
                <div className="w-9 h-9 rounded-lg bg-primary/10 text-primary flex items-center justify-center shrink-0">
                  <FileCheck className="w-4 h-4" />
                </div>
                <div>
                  <p className="font-display font-semibold text-sm">{r.name}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{r.duties}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="container mx-auto px-4 py-14 sm:py-20">
        <div className="relative overflow-hidden rounded-3xl bg-midnight px-6 py-14 text-center">
          ...
          <div className="relative">
            <h2 className="font-display text-2xl sm:text-3xl font-bold mb-3 text-primary-foreground">Ready to begin?</h2>
            ...
            <Link to="/auth">
              <Button size="lg" className="gap-2 rounded-full px-7 shadow-[var(--shadow-glow)]">
