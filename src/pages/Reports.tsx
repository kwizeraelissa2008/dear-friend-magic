  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await supabase.from("notifications").insert({
        ...
        message: `Marks have been deducted for ${approvalDialog.students?.name}. ${selectedMarks} marks deducted for: ${selectedReason}`,
        type: "approval",
      });
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Review and approve all submitted incident reports</p>
        </div>

        {isLoading ? (
          <p className="text-center py-12 text-muted-foreground">Loading reports...</p>
        ) : incidents.length === 0 ? (
          <Card><CardContent className="py-12 text-center"><p className="text-muted-foreground">No incident reports yet.</p></CardContent></Card>
        ) : (
