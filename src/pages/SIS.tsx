...
const SIS = () => {
  ...
  const [pendingDelete, setPendingDelete] = useState<ClassWithCount | null>(
    null,
  );
  ...
  const handleConfirmDelete = async () => {
    ...
    if (error) toast.error(error.message || "Failed to delete class");
      ...
      toast.success(`Class "${pendingDelete.name}" deleted`);
      fetchClasses();
    }
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between gap-2">
                      <div className="space-y-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {classItem.student_count} student
                ...
                <CardContent>
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      className="flex-1"
                    >
                      <Link to={`/sis/class/${classItem.id}`}>
                        View Students
                      </Link>
                    </Button>
                    {hasRole("dos") && (
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {pendingDelete && pendingDelete.student_count > 0 && (
                <>
                  {" "}
                  and may affect <strong>
                    {pendingDelete.student_count}
                  </strong>{" "}
                  student record{pendingDelete.student_count !== 1 ? "s" : ""}{" "}
                  assigned to it
                </>
              )}
